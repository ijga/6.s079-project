...
full = wr_standardized.merge(rb_standardized, how='outer', on=['team_name', 'Year'])
full = full.merge(te_standardized, how='outer', on=['team_name', 'Year'])
full = full.merge(ol_standardized, how='outer', on=['team_name', 'Year'])
full = full.merge(db_standardized, how='outer', on=['team_name', 'Year'])
full = full.merge(lb_standardized, how='outer', on=['team_name', 'Year'])
full = full.merge(dl_standardized, how='outer', on=['team_name', 'Year'])

columns = ['avg_diff_wr', 'avg_diff_rb', 'avg_diff_te', 'avg_diff_ol', 'avg_diff_db', 'avg_diff_lb', 'avg_diff_dl']
teams = ['ARZ', 'ATL', 'BLT', 'BUF', 'CAR', 'CHI', 'CIN', 'CLV', 'DAL', 'DEN', 'DET', 'GB', 'HST', 'IND', 'JAX', 'KC', 'LA', 'LAC', 'LV', 'MIA', 'MIN', 'NE', 'NO', 'NYG', 'NYJ', 'PHI', 'PIT', 'SEA', 'SF', 'TB', 'TEN', 'WAS']

full = full.sort_values('team_name')

full['team_names'] = full['team_name']
full = full.set_index('team_names')

for i in columns:
    for j in teams:
        mean_value = full.loc[j, i].mean()
        full.loc[j, i].fillna(value=mean_value, inplace=True)
...
new.to_csv(f'/Users/iangatlin/Desktop/6.s079_project/to_visualize/comparison_fillna.csv')

new = done.loc[done["year"].isin([2017,2018,2019,2020,2021])]
new = new.groupby('team_name').agg(({'avg_diff': 'sum', 'win_percentage': 'mean'}))
new.to_csv(f'/Users/iangatlin/Desktop/6.s079_project/to_visualize/comparison17_21_fillna.csv')

new = done.loc[done["year"].isin([2012,2013,2014,2015,2016])]
new = new.groupby('team_name').agg(({'avg_diff': 'sum', 'win_percentage': 'mean'}))
new.to_csv(f'/Users/iangatlin/Desktop/6.s079_project/to_visualize/comparison12_16_fillna.csv')
####
