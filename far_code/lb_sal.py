...
for count, i in enumerate(all_sheets):
    all_sheets[count] = i.loc[i["position"].isin(["LB"])]

for count, i in enumerate(all_sheets):
    all_sheets[count] = i.drop_duplicates(subset=['player'])

for count, i in enumerate(all_sheets):
    all_sheets[count] = i.sort_values(['team_name', 'snap_counts_run'], ascending=[True, False]).groupby('team_name').head(3)
...
# running.to_csv(f"lb_running.csv", index=False) ##

sal_cols = ['player-name', 'year signed', 'age', 'year 2', 'value', 'average/year']
sal_2007 = pd.read_csv("/Users/iangatlin/Desktop/6.s079_project/salary/lb/lb2007.csv", usecols=sal_cols)
sal_2008 = pd.read_csv("/Users/iangatlin/Desktop/6.s079_project/salary/lb/lb2008.csv", usecols=sal_cols)
sal_2009 = pd.read_csv("/Users/iangatlin/Desktop/6.s079_project/salary/lb/lb2009.csv", usecols=sal_cols)
sal_2010 = pd.read_csv("/Users/iangatlin/Desktop/6.s079_project/salary/lb/lb2010.csv", usecols=sal_cols)
sal_2011 = pd.read_csv("/Users/iangatlin/Desktop/6.s079_project/salary/lb/lb2011.csv", usecols=sal_cols)
sal_2012 = pd.read_csv("/Users/iangatlin/Desktop/6.s079_project/salary/lb/lb2012.csv", usecols=sal_cols)
sal_2013 = pd.read_csv("/Users/iangatlin/Desktop/6.s079_project/salary/lb/lb2013.csv", usecols=sal_cols)
sal_2014 = pd.read_csv("/Users/iangatlin/Desktop/6.s079_project/salary/lb/lb2014.csv", usecols=sal_cols)
sal_2015 = pd.read_csv("/Users/iangatlin/Desktop/6.s079_project/salary/lb/lb2015.csv", usecols=sal_cols)
sal_2016 = pd.read_csv("/Users/iangatlin/Desktop/6.s079_project/salary/lb/lb2016.csv", usecols=sal_cols)
sal_2017 = pd.read_csv("/Users/iangatlin/Desktop/6.s079_project/salary/lb/lb2017.csv", usecols=sal_cols)
sal_2018 = pd.read_csv("/Users/iangatlin/Desktop/6.s079_project/salary/lb/lb2018.csv", usecols=sal_cols)
sal_2019 = pd.read_csv("/Users/iangatlin/Desktop/6.s079_project/salary/lb/lb2019.csv", usecols=sal_cols)
sal_2020 = pd.read_csv("/Users/iangatlin/Desktop/6.s079_project/salary/lb/lb2020.csv", usecols=sal_cols)
sal_2021 = pd.read_csv("/Users/iangatlin/Desktop/6.s079_project/salary/lb/lb2021.csv", usecols=sal_cols)
...
salary["player_name"] = salary["player_name"].replace(to_replace = r'Shaq Quarterman', value = r'Shaquille Quarterman', regex = True)
salary["player_name"] = salary["player_name"].replace(to_replace = r"Ja'whaun Bentley", value = r"Ja'Whaun Bentley", regex = True)
salary["player_name"] = salary["player_name"].replace(to_replace = r'Anthony Walker Jr.', value = r'Anthony Walker', regex = True)
salary["player_name"] = salary["player_name"].replace(to_replace = r"Dont'a Hightower", value = r"Dont'a Hightower", regex = True)
salary["player_name"] = salary["player_name"].replace(to_replace = r'Laviska Shenault', value = r'Genard Avery', regex = True)
salary["player_name"] = salary["player_name"].replace(to_replace = r"Manti Te'o", value = r"Manti Te'o", regex = True)
salary["player_name"] = salary["player_name"].replace(to_replace = r'Jamie Collins', value = r'Jamie Collins Sr.', regex = True)
salary["player_name"] = salary["player_name"].replace(to_replace = r'Shaun-Dion Hamilton', value = r'Shaun Dion Hamilton', regex = True)
salary["player_name"] = salary["player_name"].replace(to_replace = r'Thomas Davis', value = r'Thomas Davis Sr.', regex = True)
salary["player_name"] = salary["player_name"].replace(to_replace = r'Telvin Smith', value = r'Telvin Smith Sr.', regex = True)
salary["player_name"] = salary["player_name"].replace(to_replace = r'Navorro Bowman', value = r'NaVorro Bowman', regex = True)
...
avg = full.copy()
# avg.to_csv(f"lb_test.csv", index=False) ##

average = pd.DataFrame(columns = ['position','player_name', 'team_name', 'grades_defense', 'snap_counts_run', 'rank',
...
for count, data in enumerate(avg.iterrows()):
    ...
    if countem == 0:
        average.loc[len(average.index)] = [row['position'], row['player_name'], row['team_name'], row['grades_defense'],
                                           ...
                                           avg.iloc[count:count + 4, 14].mean()]
        continue
    if countem == 1:
        average.loc[len(average.index)] = [row['position'], row['player_name'], row['team_name'], row['grades_defense'],
                                       row['snap_counts_run'], row['rank'], row['Year'], row['player_name'],
                                       row['year_signed'], row['age'], row['average/year'],
                                       avg.iloc[count - 1:count + 4, 14].mean()]
        continue
    if countem == 2:
        average.loc[len(average.index)] = [row['position'], row['player_name'], row['team_name'], row['grades_defense'],
                                       row['snap_counts_run'], row['rank'], row['Year'], row['player_name'],
                                       row['year_signed'], row['age'], row['average/year'],
                                       avg.iloc[count - 2:count + 4, 14].mean()]
        continue
    if countem == 3:
        average.loc[len(average.index)] = [row['position'], row['player_name'], row['team_name'], row['grades_defense'],
                                       row['snap_counts_run'], row['rank'], row['Year'], row['player_name'],
                                       row['year_signed'], row['age'], row['average/year'],
                                       avg.iloc[count - 3:count + 4, 14].mean()]
        continue
    else:
        average.loc[len(average.index)] = [row['position'], row['player_name'], row['team_name'], row['grades_defense'],
                                           row['snap_counts_run'], row['rank'], row['Year'], row['player_name'],
                                           row['year_signed'], row['age'], row['average/year'],
                                           avg.iloc[count-4:count+4, 14].mean()]

average['diff'] = average['average/year'] - average['sim_sal']

average.to_csv(f"lb_something.csv", index=False) ##

teams = average.copy()
end = teams.groupby(['team_name', 'Year'], as_index=False).agg(avg_diff_lb=('diff', 'mean'))
end.to_csv(f'/Users/iangatlin/Desktop/6.s079_project/final/lb_end.csv', index=False) ##
